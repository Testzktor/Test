...
const userIdentificationString = function(req, res, next) {
           var userString=   (Math.random() + 1).toString(36).substring(7)+ Date.now() ;
           const hash = encrypt(userString);
        	db.query(`SELECT count(*) totalRecords from tbl_user_profile where user_identification_key = ?`, [hash], function(error, results, fields) {
		if (error) {
			res.userIdentificationString =hash;
			next();
		} else {
			...
			next();
		    	  ...
		    	  var userString=   (Math.random() + 1).toString(36).substring(7)+ Date.now() ;
                  res.userIdentificationString =encrypt(userString);
		      	next();
		    	}
		}
	});
